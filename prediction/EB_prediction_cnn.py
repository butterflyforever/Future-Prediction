...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename='../data/all/1000/model/ONE_EB_CNN.log',
                    filemode='w')
...
def EB_CNN(data_path, result_path, n_epoch, input_dim):
    # load data
    ...
    f = open("../data/all/1000/model/ONE_EB_CNN_result.txt", 'a')
...
def file_filter(count_res_path):
    ...
    with open(count_res_path, 'r', encoding='utf8') as p:
        for line in p:
            ...
            if line == "":
                continue
            if abs(float(line.split('\t')[2]) - 0.4941176533) >= 0.000001:
                if len(line.split('\t')) > 5:
...
if __name__ == '__main__':
    ...
    result_path = "../data/all/1000/model/hdf5/ONE_EB_CNN/EB_CNN_%s.hdf5"
    PRICE_PATH = "../data/all/price.txt"
    count_res_path = "../data/all/1000/model/ONE_EB_CNN_result.txt"
    ...
    for i, product in enumerate(products):
        print("EB_CNN No.%d, %s" % (i, product))
        logging.info("EB_CNN No.%d, %s" % (i, product))
        # product = 'JM'
        try:
            EB_CNN(data_path % product, result_path % product, n_epoch, input_dim)
            # break
